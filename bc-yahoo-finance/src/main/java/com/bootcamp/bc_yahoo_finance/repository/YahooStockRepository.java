...
@Query(value = "SELECT * FROM TSTOCK_QUOTE_YAHOO WHERE symbol = :symbol AND market_time BETWEEN :marketTime AND :marketTimeMax AND data_type = :dataType ORDER BY market_time ASC", nativeQuery = true)
Optional<List<YahooStockEntity>> findAllStockBySymbolByDate(@Param("symbol") String symbol, @Param("marketTime") Long marketTime, @Param("marketTimeMax") Long max, @Param("dataType") String dataType);
